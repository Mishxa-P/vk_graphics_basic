...
void SimpleShadowmapRender::AllocateResources()
{
  ...
  gaussCoeffs    = m_context->createBuffer(etna::Buffer::CreateInfo{
       .size        = sizeof(float) * (m_blur_window_size * 2 + 1),
       .bufferUsage = vk::BufferUsageFlagBits::eUniformBuffer,
  ...
  std::vector<float> coeffs(m_blur_window_size * 2 + 1, 0.0f);
  float sigma  = m_blur_window_size / 3.0f;
  float sigma2 = 2 * sigma * sigma;
  for (int i = 0; i < m_blur_window_size * 2 + 1; i++)
  {
    coeffs[i] = exp(-(i - m_blur_window_size) * (i - m_blur_window_size) / sigma2);
  }
...
void SimpleShadowmapRender::BuildCommandBufferSimple(VkCommandBuffer a_cmdBuff, VkImage a_targetImage, VkImageView a_targetImageView)
{
  ...
  etna::set_state(a_cmdBuff, preBlurImage.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderRead, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
  etna::set_state(a_cmdBuff, finalImage.get(), vk::PipelineStageFlagBits2::eComputeShader, vk::AccessFlagBits2::eShaderWrite, vk::ImageLayout::eGeneral, vk::ImageAspectFlagBits::eColor);
  etna::finish_frame(a_cmdBuff);


  //gaussian_blur
  {
    auto blurInfo = etna::get_shader_program("gaussian_blur");
    auto set      = etna::create_descriptor_set(blurInfo.getDescriptorLayoutId(0), a_cmdBuff, { etna::Binding{ 0, preBlurImage.genBinding(defaultSampler.get(), vk::ImageLayout::eGeneral) }, etna::Binding{ 1, finalImage.genBinding(defaultSampler.get(), vk::ImageLayout::eGeneral) }, etna::Binding{ 2, gaussCoeffs.genBinding() } });
  ...
  etna::set_state(a_cmdBuff, finalImage.get(), vk::PipelineStageFlagBits2::eTransfer, vk::AccessFlagBits2::eTransferRead, vk::ImageLayout::eTransferSrcOptimal, vk::ImageAspectFlagBits::eColor);
